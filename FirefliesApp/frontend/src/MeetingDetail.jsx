...
//   return (
//     <div style={{ padding: "24px", maxWidth: "1000px", margin: "auto", fontFamily: "'Inter', sans-serif", backgroundColor: '#304355ff', color: '#0f172a' }}>
...
// import { useEffect, useState, useRef } from "react";
// import { useParams, useNavigate } from "react-router-dom";
// import { Download, ArrowLeft, Save, RefreshCw, Check, Settings2, FileText, Calendar, Clock, Zap } from 'lucide-react';
// import ReactMarkdown from "react-markdown";
// import remarkGfm from "remark-gfm";
...
// const CustomCheckbox = ({ label, checked, onChange }) => (
//   <label onClick={onChange} className="flex items-center gap-3 cursor-pointer text-slate-300 hover:text-white transition-colors">
...
//       <div
//         key={i}
//         className="h-24 bg-slate-800/30 backdrop-blur-md rounded-xl border border-slate-700/20 animate-pulse"
//       />
//     ))}
//   </div>
// );
...
//   if (loading || !summaryPreferences) return <LoadingSkeleton />;
//   if (!meeting) return <div className="text-center mt-16 text-slate-300">Meeting not found.</div>;

//   const d = parseDate(meeting.date);
//   const audioSrc = meeting?.audio_url ? (meeting.audio_url.startsWith('http') ? meeting.audio_url : `${API}${meeting.audio_url}`) : null;

//   return (
//     <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900/60 to-purple-900/70 text-white p-6">
...
//         {/* Header */}
//         <div className="bg-slate-800/40 backdrop-blur-xl rounded-3xl  p-6 mb-8 border border-slate-700/40 shadow-lg">
//           <div className="flex flex-col lg:flex-row justify-between items-start lg:items-start gap-6">
//             <div className="flex items-center gap-4">
//               <button
//                 onClick={() => navigate(-1)}
//                 className="p-3 bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 rounded-2xl transition-all duration-300 hover:-translate-y-0.5"
//               >
//                 <ArrowLeft size={20} />
//               </button>
//               <div className="p-3 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-2xl shadow-md">
...
//             <div className="bg-slate-800/40 backdrop-blur-md rounded-2xl p-6 border border-slate-700/30 h-full">
...
//                   <div 
//                     key={key} 
//                     className="p-4 rounded-xl border border-slate-700/30 bg-slate-700/20 hover:bg-slate-700/30 transition-all duration-200 cursor-pointer"
//                     onClick={() => setSummaryPreferences(prev => ({ ...prev, [key]: !prev[key] }))}
//                   >
//                     <CustomCheckbox
//                       label={key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
...
//               {summaryPreferences.extended_sections && Object.keys(summaryPreferences.extended_sections).length > 0 && (
//                 <div className="border border-slate-700/30 rounded-2xl p-5 bg-slate-700/20">
//                   <h4 className="font-semibold text-slate-200 mb-4">Detailed Sections</h4>
//                   <div className="space-y-3">
//                     {Object.keys(summaryPreferences.extended_sections).map((title) => (
//                       <div 
//                         key={title}
//                         className="p-3 rounded-lg border border-slate-700/30 bg-slate-700/10 hover:bg-slate-700/20 transition-all duration-200 cursor-pointer"
//                         onClick={() => {
//                           setSummaryPreferences(prev => ({
//                             ...prev, 
//                             extended_sections: { 
...
//                 className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 rounded-xl transition-all duration-300"
//               >
//                 <RefreshCw size={16} />
//                 Regenerate from Selected
//               </button>
...
//                     className="p-2 bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 rounded-lg transition-all duration-300"
...
//               {previewMode ? (
//                 <div className="bg-slate-700/20 border border-slate-700/30 rounded-xl p-6 min-h-64">
//                   <ReactMarkdown
//                     remarkPlugins={[remarkGfm]}
//                     components={{
//                       h2: ({ node, ...props }) => {
...
//                             else if (text.includes("Key Points")) bg = "#9333ea";
//                         }
//                         return <h2 {...props} className="inline-block px-4 py-2 rounded-lg text-white text-sm font-semibold mb-3 mt-4" style={{ backgroundColor: bg }} />;
//                       },
//                       p: ({ node, ...props }) => <p {...props} className="text-slate-300 leading-relaxed mb-4" />,
//                       ul: ({ node, ...props }) => <ul {...props} className="text-slate-300 list-disc list-inside mb-4 space-y-1" />,
...
//                   value={summary}
//                   onChange={(e) => setSummary(e.target.value)}
//                   className="w-full min-h-64 bg-slate-700/30 border border-slate-600/40 rounded-xl p-4 text-slate-200 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/50"
//                 />
//               )}
...
//                   disabled={!dbId}
//                   className="flex-1 bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 text-white px-6 py-3 rounded-xl font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
//                 >
//                   Next
//                 </button>
//               </div>
...
//                 className="w-full bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 px-4 py-3 rounded-xl text-slate-300 transition-all duration-300 flex items-center justify-between"
//               >
...
//               {transcriptOpen && (
//                 <div className="bg-slate-800/40 backdrop-blur-md rounded-2xl p-6 border border-slate-700/30 mt-4">
//                   {audioSrc && (
//                     <div className="mb-6">
//                       <h4 className="text-lg font-semibold text-white mb-3">Listen to Recording</h4>
...
//                       {meeting.sentences.map((s) => (
//                         <div key={s.index} className="border-b border-slate-700/30 pb-4 last:border-b-0">
//                           <div className="text-blue-400 font-medium mb-1">{s.speaker_name ?? "Speaker"}</div>
...
// import { useEffect, useState, useRef } from "react";
// import { useParams, useNavigate } from "react-router-dom";
// import { Download, ArrowLeft, Save, RefreshCw, Check, Settings2, FileText, Calendar, Clock, Zap, Play, Headphones, Edit, Eye } from 'lucide-react';
// import ReactMarkdown from "react-markdown";
// import remarkGfm from "remark-gfm";
// import FileSelectionModal from './FileSelectionModal';
...
// const CustomCheckbox = ({ label, checked, onChange }) => (
//   <label onClick={onChange} className="flex items-center gap-3 cursor-pointer text-slate-300 hover:text-white transition-colors group">
//     <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 group-hover:border-blue-400 ${
//       checked 
//         ? 'bg-blue-500 border-blue-500' 
//         : 'bg-slate-700/50 border-slate-600'
//     }`}>
...
//         {/* Header */}
//         <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl p-6 mb-6 border border-slate-700/40 shadow-lg">
//           <div className="flex flex-col md:flex-row justify-between items-start  gap-4">
//             <div className="flex items-center gap-4 items-center">
//               <button
//                 onClick={() => navigate(-1)}
//                 className="p-3  bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 rounded-xl transition-all duration-300 hover:-translate-y-0.5"
//               >
//                 <ArrowLeft size={20} />
//               </button>
//               <div className="p-3 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-xl shadow-md">
//                 <FileText size={24} className="text-white" />
//               </div>
...
//                   {meeting.title}
//                 </h1>
//                 <div className="flex flex-wrap items-center gap-4 mt-2 text-slate-300 text-sm">
//                   <div className="flex items-center gap-2 bg-slate-700/40 px-3 py-1 rounded-lg">
//                     <Calendar size={14} />
//                     <span>{d ? d.toLocaleDateString(undefined, { dateStyle: "long" }) : ""}</span>
//                   </div>
//                   <div className="flex items-center gap-2 bg-slate-700/40 px-3 py-1 rounded-lg">
...
//                 <h4 className="text-slate-400 text-sm font-medium uppercase tracking-wide mb-3">Core Sections</h4>
//                 {['overview', 'action_items', 'keywords', 'bullet_gist'].map((key) => (
//                   <div 
//                     key={key} 
//                     className="p-3 rounded-xl border border-slate-700/30 bg-slate-700/20 hover:bg-slate-700/30 transition-all duration-200 cursor-pointer"
//                     onClick={() => setSummaryPreferences(prev => ({ ...prev, [key]: !prev[key] }))}
//                   >
//                     <CustomCheckbox
//                       label={key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
...
//               {summaryPreferences.extended_sections && Object.keys(summaryPreferences.extended_sections).length > 0 && (
//                 <div className="border border-slate-700/30 rounded-2xl p-4 bg-slate-700/20">
//                   <h4 className="text-slate-400 text-sm font-medium uppercase tracking-wide mb-3">Extended Sections</h4>
//                   <div className="space-y-2">
//                     {Object.keys(summaryPreferences.extended_sections).map((title) => (
//                       <div 
//                         key={title}
//                         className="p-2 rounded-lg border border-slate-700/30 bg-slate-700/10 hover:bg-slate-700/20 transition-all duration-200 cursor-pointer"
//                         onClick={() => {
//                           setSummaryPreferences(prev => ({
//                             ...prev, 
//                             extended_sections: { 
...
//               <button 
//                 onClick={handleRegenerateSummary}
//                 className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-xl transition-all duration-300 font-medium"
//               >
//                 <RefreshCw size={16} />
//                 Regenerate Summary
//               </button>
...
//             <button 
//               onClick={() => setTranscriptOpen(!transcriptOpen)}
//               className="w-full mt-4 bg-slate-800/40 hover:bg-slate-700/40 border border-slate-700/30 px-4 py-3 rounded-xl text-slate-300 transition-all duration-300 flex items-center justify-between group"
//             >
//               <div className="flex items-center gap-2">
//                 <Headphones size={18} />
//                 <span>Meeting Transcript</span>
//               </div>
//               <span className="group-hover:text-white">{transcriptOpen ? "▲" : "▼"}</span>
//             </button>
//           </div>
...
//               {previewMode ? (
//                 <div className="bg-slate-700/20 border border-slate-700/30 rounded-xl p-6 min-h-96">
//                   <ReactMarkdown
//                     remarkPlugins={[remarkGfm]}
//                     components={{
//                       h2: ({ node, ...props }) => {
...
//                             else bg = "#475569";
//                         }
//                         return <h2 {...props} className="inline-block px-4 py-2 rounded-lg text-white text-sm font-semibold mb-4 mt-6 first:mt-0" style={{ backgroundColor: bg }} />;
//                       },
//                       p: ({ node, ...props }) => <p {...props} className="text-slate-300 leading-relaxed mb-4" />,
//                       ul: ({ node, ...props }) => <ul {...props} className="text-slate-300 list-disc list-inside mb-4 space-y-2" />,
//                       li: ({ node, ...props }) => <li {...props} className="text-slate-300" />
//                     }}
//                   >
//                     {summary}
...
//                   value={summary}
//                   onChange={(e) => setSummary(e.target.value)}
//                   className="w-full min-h-96 bg-slate-700/30 border border-slate-600/40 rounded-xl p-4 text-slate-200 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/50 font-mono text-sm"
//                   placeholder="Start editing your meeting summary..."
//                 />
//               )}
...
//                   onClick={() => setShowFileSelectionModal(true)}
//                   disabled={!dbId}
//                   className="flex-1 bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600/40 text-white px-6 py-3 rounded-xl font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
//                 >
//                   <FileText size={18} />
//                   Generate Files
//                 </button>
...
//             {/* Transcript Section */}
//             {transcriptOpen && (
//               <div className="bg-slate-800/40 backdrop-blur-md rounded-2xl p-6 border border-slate-700/30 mt-6">
...
//                         </div>
//                         <p className="text-slate-300 ml-4">{s.text}</p>
//                       </div>
//                     ))}
//                   </div>
//                 ) : (
...
//       <div className="container mx-auto relative z-10 max-w-7xl p-4 md:p-6 space-y-6">
//         {/* Enhanced Header */}
...
//                     <div className="flex items-center gap-2 bg-slate-700/50 backdrop-blur-sm px-4 py-2 rounded-xl border border-slate-600/30 hover:bg-slate-700/70 transition-all duration-300">
...
//                     { key: 'bullet_gist', label: 'Key Points', icon: '💡' }
//                   ].map(({ key, label, icon }) => (
//                     <div 
//                       key={key} 
...
//               <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-slate-700/30">
//                 <ActionButton
//                   variant="primary"
//                   onClick={handleSaveAll}
//                   disabled={!dbId || saving}
...
//                       <div 
//                         key={s.index || index} 
//                         className="group hover:bg-slate-700/20 rounded-xl p-4 transition-all duration-300 border border-transparent hover:border-slate-600/30"
//                       >
