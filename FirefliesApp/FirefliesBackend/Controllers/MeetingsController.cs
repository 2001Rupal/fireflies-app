...
//             if (request.Temperature < 0.1 || request.Temperature > 1.0)
//         return BadRequest("Temperature must be between 0.1 and 1.0.");
...
//                 meeting.ProjectPlanTemperature = request.Temperature;
    ...
    public class MeetingsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpsertMeeting([FromBody] SaveMeetingDto dto)
        {
            ...
            try
            {
                ...
                if (dto.SentimentPositivePct.HasValue)
                    meeting.SentimentPositivePct = dto.SentimentPositivePct;
                if (dto.SentimentNeutralPct.HasValue)
                    meeting.SentimentNeutralPct = dto.SentimentNeutralPct;
                if (dto.SentimentNegativePct.HasValue)
                    meeting.SentimentNegativePct = dto.SentimentNegativePct;
                if (!string.IsNullOrWhiteSpace(dto.AnalyticsJson))
                    meeting.AnalyticsJson = dto.AnalyticsJson;
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            return Ok(new
            {
                ...
                m.TranscriptJson,
                hasProjectPlan = !string.IsNullOrEmpty(m.ProjectPlan),
                analytics = m.AnalyticsJson != null ? JsonSerializer.Deserialize<object>(m.AnalyticsJson) : new
                {
                    sentiments = new
                    {
                        positive_pct = m.SentimentPositivePct,
                        neutral_pct = m.SentimentNeutralPct,
                        negative_pct = m.SentimentNegativePct
                    }
                }
            });
        }
